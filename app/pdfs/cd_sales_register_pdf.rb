class CdSalesRegisterPdf < Prawn::Document
	def initialize(from, to, total_sale, total_cd_sale, cd_sale)
		super()
		font "Times-Roman"
		@items =Array.new
...
#		text "Triplicane, Chennai - 03.", :size => 10, :align => :left
		...
		draw_a_line()
		move_down(20)
		text "CD Sales Register: #{@from.strftime('%d %b')} - #{@to.strftime('%d %b, %Y')}", :size => 18
		move_down(25)
...
#		text "ORDER NO: #{@sale.customer.id if @sale.customer}", :size => 10, :align => :right
	...
	def line_item_rows
		@items.push ["Date","Day", "CD Value"]
		@total_sale.each.map do |k,v|
			@items.push [k.strftime('%d-%m-%Y'), k.strftime("%A"),  sprintf('%.2f', @cd_sale[k])]
		end
		@items.push ["Total", "", sprintf('%.2f', @total_cd_sale)]
