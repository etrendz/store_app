class CbPurchaseRegisterPdf < Prawn::Document
	def initialize(from, to, total_purchase, cb_purchase)
		super()
		font "Times-Roman"
		@items =Array.new
...
#		text "Triplicane, Chennai - 03.", :size => 10, :align => :left
		...
		draw_a_line()
		move_down(20)
		text "CB Purchase Report: #{@from.strftime('%d %b')} - #{@to.strftime('%d %b, %Y')}", :size => 18
		move_down(12)
		line_items
		move_down(20)
