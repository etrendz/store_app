class CbSalesRegisterPdf < Prawn::Document
	def initialize(from, to, total_sale, total_cb_sale, cb_sale)
		super()
		font "Times-Roman"
		@items =Array.new
...
#		text "Triplicane, Chennai - 03.", :size => 10, :align => :left
		...
		draw_a_line()
		move_down(20)
		text "CB Sales Register: #{@from.strftime('%d %b')} - #{@to.strftime('%d %b, %Y')}", :size => 18
		move_down(25)
...
#		text "ORDER NO: #{@sale.customer.id if @sale.customer}", :size => 10, :align => :right
	...
	def line_item_rows
		...
		@total_sale.each.map do |k,v|
			@items.push [k.strftime('%d-%m-%Y'), k.strftime("%A"),  sprintf('%.2f', @cb_sale[k])]
		end
		@items.push ["Total", "", sprintf('%.2f', @total_cb_sale)]
