class PurchaseRegisterPdf < Prawn::Document
	def initialize(from, to, total_purchase, purchase)
		super()
		font "Times-Roman"
		@items =Array.new
...
#		text "Triplicane, Chennai - 03.", :size => 10, :align => :left
		...
		draw_a_line()
		move_down(20)
		text "Purchase Report: #{@from.strftime('%d %b')} - #{@to.strftime('%d %b, %Y')}", :size => 18
		move_down(12)
		line_items
		move_down(20)
	...
	def line_item_rows
		@items.push ["Date","Day", "Purchase"]
		@purchase.each.map do |k,v|
