...
#  layout 'user_layout'
  ...
  def new
    ...
    @purchase = Purchase.new(:invoice => invoice)
		if Supplier.last.nil?
			redirect_to new_supplier_path, notice: 'Supplier Not Available. Please create a one.'
		else
			respond_to do |format|
  ...
  def add_purchase_products
	...
	render :partial => 'purchase_product', :locals => {:purchase_product_object => PurchaseProduct.new(:product_id => @product.id, :cprice => @cprice, :price => @price, :qty => @qty)}
  end
...
#	@purchases = @purchases.where(:supplier_id => params[:supplier]) if params[:supplier]
  ...
  private
    ...
    def purchase_params
      params.require(:purchase).permit(:invoice, :paid, :purchase_date, :supplier_id, :purchase_products_attributes, new_purchase_product_attributes:[:id, :price, :cprice, :product_id, :purchase_id, :qty])
    end
end
