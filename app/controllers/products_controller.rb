class ProductsController < ApplicationController
  ...
  # GET /products.json
	...
	def home
		...
		month_sales = Sale.includes(:sale_products).where(:sale_date => [(Date.today - 1.month)..Date.today])
		week_sales = month_sales.where(:sale_date => [(Date.today - 1.week)..Date.today])
		day_sales = week_sales.where(:sale_date => Date.today)
		...
		# calculating the weekly sales
		week_sales.each do |s|
			total_sales = s.sale_products.sum(:price)
			discount = (total_sales * (s.discount).to_i / 100)
			vat = (total_sales - discount) * (s.vat).to_i / 100
...
#			@day_sales += s.sale_products.map(&:price).sum
			@day_sales += (total_sales - discount + vat)
		end
			...
			@categories.each { |category| 
			  ...
			  category.products.each { |product| 
					product.sale_products.each { |sp|
						@price[category.id] = @price[category.id] + sp.price if @this_month_sales.include?sp.sale 
					}
			  }
  ...
  def index
    @products = Product.where(:category_id => params[:category] || 1)
		@products = Product.where("name like ?", "%#{params[:term]}%") if params[:term]
		@category = Category.find(params[:category]) || Category.find(1)
		@total_cprice = 0 
  ...
  def products_name
		@products = Product.order(params[:sort]).where("name like ?", "%#{params[:term]}%").paginate(:page => params[:page], :per_page => 15)

    respond_to do |format|
      format.html # index.html.erb
      format.json { render json: @products.limit(10).map(&:name) }
    end
  end
 ...
 #   @product = Product.find(params[:id])
    @product = Product.includes({purchase_products: :purchase}, {sale_products: :sale}, :stock_products).find(params[:id])
...
#	p ("bcode"+i.to_s).to_sym
  ...
  labels = Prawn::Labels.render(names, :type => "Agipa119461") do |pdf, name|
		...
		if product
			barcode.annotate_pdf(pdf, {:y => 10, :height => 20, :xdim => 0.75, :ydim => 0.75})
		else
			pdf.text ' '
		end
		pdf.text product.name, :align => :left, :valign => :bottom, :size => 6 if product
		pdf.text "Rs."+product.stock_products.last.price.to_s, :align => :center, :valign => :bottom, :size => 6 if product && product.stock_products.last
		pdf.text name, :align => :right, :valign => :bottom, :size => 6
#	pdf.text name
  end
  ...
  private
    ...
    def product_params
      params.require(:product).permit(:category_id, :name, :qty, :unit, :code, :_delete, :sold_products_attributes)
    end
end
