...
#	pdf.text name
  ...
  labels = Prawn::Labels.render(names, :type => "Agipa119461", document: {:print_scaling => :none}) do |pdf, name|
	  product = Product.find_by_code(name)
		...
		barcode = Barby::Code25Interleaved.new name
			if product
				barcode.annotate_pdf(pdf, {:y => 36, :x => 25, :height => 19, :xdim => 1.0, :ydim => 1.0})
			else
				pdf.text ' '
			end	
			pdf.move_up(4)
  ...
  def search_products
	...
	def stock_report
	  ...
	  @to = Date.strptime(params[:to])
		...
		elsif params[:report_type] == 'category_wise'
			render :partial => 'category_wise_report'
		else			
    respond_to do |format|
      format.html {render :partial => 'stock_report'}
			format.pdf do
				pdf = StockReportPdf.new(@from, @to, @opening_stock, @purchase_stock, @sale_stock, @sale_return_stock)
