class PurchaseProductsController < ApplicationController
  ...
  def create
    @purchase_product = PurchaseProduct.new(params[:purchase_product])
  ...
  def update
    respond_to do |format|
      if @purchase_product.update_attributes(params[:purchase_product])
  ...
  private
    ...
    def purchase_product_params
      params.require(:purchase_product).permit(:price, :cprice, :product_id, :purchase_id, :qty)
    end
end
